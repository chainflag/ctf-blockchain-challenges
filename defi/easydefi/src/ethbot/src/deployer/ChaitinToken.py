...
def chaitintoken(w3, _abi, _bin, _acct, _acc_nonce):
...
def review(ctx, _acct, _txhash):
    w3 = ctx['web3instance']
    p.ppln("[-] Check for chaitintoken contract successful deployment... ")
    if 'chaitintoken' not in ctx['deployedcontracts'][_acct.address].keys():
        try:
        ...
        update_user_deploy_contracts(ctx, _acct.address, 'chaitintoken', addr)
    else:
        addr = ctx['deployedcontracts'][_acct.address]['chaitintoken']

    if not addr:
        p.pln(p.in_fg_color("[!] contract chaitintoken deploy Broken down", r.red5))
    else:
        p.ppln("[+] contract chaitintoken deploy " + p.in_fg_color("success", r.green1))
        p.ppln("[!] chaitintoken contract address is " + p.in_fg_color(addr, r.blue1))
